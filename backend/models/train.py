...
parameters_default = {
    'nb_classes': 3,
    'x_columns': ['time', 'Fp1', 'Fp2', 'F3', 'F4', 'C3', 'C4', 'P3', 'P4', 'O1', 'O2',
...
xeeg_model = selfeeg.models.xEEGNet(nb_classes=parameters_default['nb_classes'], Chans=parameters_default['Chans'],
                                     Samples=parameters_default['sample_length'], Fs=parameters_default['srate'], global_pooling=True)

participants_ids_all = list(np.arange(1, 89))
participants_ids_small = [
    1,2,3,4,5,6,
    40,41,42,43,44,45,
    80,81,82,83,84,85
]
...
def train_save_model(model_path: str,
                dir_data: str,
    ...
    df_eeg = load_multiple_eegfiles(dir_data, participant_ids_train + participant_ids_val, gen_filename, df_metadata, n_max=n_max)
    ...
    trainloader = get_data_loader(df_train, parameters['Chans'], parameters['sample_length'], 
                                  parameters['batchsize'], parameters['workers'], x_columns=parameters['x_columns'])
    valloader = get_data_loader(df_val, parameters['Chans'], parameters['sample_length'], 
                                parameters['batchsize'], parameters['workers'], x_columns=parameters['x_columns'])
