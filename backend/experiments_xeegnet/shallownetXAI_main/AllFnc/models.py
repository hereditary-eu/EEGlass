...
class ShallowNetEncoder2(nn.Module):
    def __init__(
        ...
        if self.Fs <= 0 and not (random_temporal_filter):
            raise ValueError("to properly initialize non random temporal fir filters, Fs (sampling rate) must be given")
        ...
        if log_activation_base in ["e", torch.e]:
            ...
            raise ValueError(
                "allowed activation base are 'e' for torch.log, '10' for torch.log10, and 'dB' for 10*torch.log10"
            )
    ...
    def _initialize_custom_temporal_filter(self, seed=None):
        ...
        for i in range(np.min([F, len(comb)])):
            filt_coeff = firwin(KernLength, self._merge_tuples(comb[i]), pass_zero=False, fs=self.Fs)
...
class CustomDenseLayer(nn.Module):
    def __init__(self, input_size, output_size, connections, initial_weights):
        ...
        self.weights = nn.Parameter(
            torch.tensor(initial_weights, dtype=torch.float32).view(-1, 1), requires_grad=False
        )  # Frozen weights
