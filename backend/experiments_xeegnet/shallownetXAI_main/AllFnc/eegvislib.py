...
def get_extended_lims(lims, percentage=5):
    ...
    return [lims[0] * factor - lims[1] * (factor - 1), lims[1] * factor - lims[0] * (factor - 1)]
...
####    FUNCTIONS FOR INPUT     ###
###################################
def scalpPSD_freq(input_window, spec_dict, inout_labels, batch_ID):
    """
    ...
    fig.suptitle(
        f"Batch Index: {batch_ID}| {inout_labels[0][batch_ID]} -> {inout_labels[1][batch_ID]}",
    ...
    lims = get_vlimscalp(input_window, input_window_batch, spec_dict["srate"], spec_dict["flim"], num_freqs, spec_dict)
    ...
    for j in range(num_freqs):
        ...
        evoked_data.plot_topomap(
            show=False,
            axes=(ax[2 * j], ax[2 * j + 1]),
            show_names=True,
            scalings=1,
            vlim=get_extended_lims(lims[j, :]),
            cmap=spec_dict["cmap"],
...
def scalpTime(input_window, Mdl, layer, spec_dict, inout_labels, batch_ID, FM_ID):
    ...
    fig, axs = plt.subplots(
        fm,
        lfig * 2,
        figsize=((lfig - 1) * 2 * spec_dict["figdim"][0], spec_dict["figdim"][1] * fm),
    ...
    for k, fm_id in enumerate(fm_IDs):
        if layer is not None:
            ...
            data_vector = torch.squeeze(
                convoluted_output.index_select(0, torch.tensor(batch_ID)).index_select(1, torch.tensor(fm_id))
            ).numpy()
        else:
            title_fm = "Input"
        ...
        for i in range(lfig - 1):
            evoked_data.plot_topomap(
                spec_dict["times"][i],
                show=False,
                axes=(axs[k, 2 * i], axs[k, 2 * i + 1]),
                show_names=True,
                cmap=spec_dict["cmap"],
                scalings=1,
                vlim=get_extended_lims(lims),
                contours=spec_dict["contours"],
            )
            ...
            axs[k, 2 * i].set_title(
                title_fm + " | Time: " + axs[k, 2 * i].title.get_text(), fontsize=spec_dict["font"]
        ...
        _, p_values_corrected, _, _ = multitest.multipletests(
            p_values_flat, alpha=spec_dict["alpha"], method=spec_dict["method"]
        )
        ...
        im = axs[k, -2].imshow(significant_corr, cmap=spec_dict["cmap"], vmin=-1, vmax=1, interpolation="nearest")
        ...
        axs[k, -2].set_xticklabels(
            spec_dict["channels"], fontsize=spec_dict["font"] - 4, rotation=spec_dict["rotation"]
        )
...
def temporalFM_plot(ax, f_IN, Pxx_IN, f_FM, Pxx_FM, ch_dict, chan, FM_string, spec_dict):
    ...
    ax.legend(fontsize=spec_dict["font"] - 6, loc=spec_dict["loc"], bbox_to_anchor=(1.4, 1), borderaxespad=0.0)
    ax.grid("both")
    return ax


#### PLOT THE FM 1 LAYER IN THE FREQUENCY DOMAIN (ELECTRODES) ####
def channelsPSD_lines(input_window, Mdl, layer, spec_dict, inout_labels, chan_ID, batch_ID, FM_ID=None):
    """
...
def scalpPSD_band(input_window, Mdl, layer, spec_dict, inout_labels, batch_ID, FM_ID=None):
    """
    ...
    lims = get_vlimscalp(
        convoluted_output, convoluted_output_batch, spec_dict["srate"], spec_dict["flim"], num_freqs, spec_dict
    )
    ...
    for i, fm_id in enumerate(FM_indices):
        ...
        for j, (band_name, band_range) in enumerate(spec_dict["bands"].items()):
            ...
            ax_current[1].set_title(
                f"PSD$_{{{unit}}}$", fontsize=spec_dict["font"] - 4
            )  # TO DO (UNIT BASED ON LOGNORM)
...
def BP_plot(ax, i, h, angles, f_hz, spec_dict, string, maxminv, ind):
    ...
    if spec_dict["bodeplot"] in ["mag", "both"]:
        ax[i].plot(
            f_hz,
            data_transform(np.abs(h) + np.finfo(float).eps, spec_dict["lognorm"]),
            color=spec_dict["colors"][ind][0],
            linewidth=spec_dict["linew"],
            linestyle=spec_dict["linestyle"][ind],
        )
...
def horizontalKernel1D_bode(
    Mdl_weights, layer, spec_dict, filter_ID=None, kernel_ID=0, kernel_height=0, MdlBase_weights=None
):
    """
    ...
    legend1_elements = [
        Line2D([0], [0], color="k", label="After Train", linestyle=spec_dict["linestyle"][0]),
    ...
    for f_id in filter_indices:
        kernel_weights = (
            weights.index_select(0, torch.tensor(f_id))
            .index_select(1, torch.tensor(kernel_ID))
            .index_select(2, torch.tensor(kernel_height))
        )
    ...
    if base_weights is not None:
        for f_id in filter_indices:
            basekernel_weights = (
                base_weights.index_select(0, torch.tensor(f_id))
                .index_select(1, torch.tensor(kernel_ID))
                .index_select(2, torch.tensor(kernel_height))
            )
...
#### STEM PLOT ####
def horizontalKernel1D_stem(Mdl_weights, layer, spec_dict, filter_ID=None, kernel_ID=0, kernel_height=0):
    """
    ...
    for i, f_id in enumerate(filter_IDs):
        ...
        ax1.imshow(
            torch.squeeze(kernel_weights, dim=(0, 1)),
            cmap=spec_dict["cmap"],
            aspect="auto",
            interpolation="nearest",
            vmin=spec_dict["vlim"][0],
            vmax=spec_dict["vlim"][1],
        )  # 2D heatmap
...
###################################
def channelsPSD_box(input_window, Mdl, layer, spec_dict, inout_labels, batch_ID, FM_ID=None, chan_ID=None):
    """
    ...
    if (fm == 1) and (chan > 1):
        ...
        for i, ch_id in enumerate(ch_IDs):
            time_w = torch.squeeze(
                convoluted_output.index_select(0, torch.tensor(batch_ID))
                .index_select(1, torch.tensor(fm_IDs[0]))
                .index_select(2, torch.tensor(ch_id))
            )
            f, Pxx = get_Pxx(time_w, spec_dict["srate"])
            Pxx_tensor[i, :] = Pxx
            ytick = chan
        ...
        for i, fm_id in enumerate(fm_IDs):
            time_w = torch.squeeze(
                convoluted_output.index_select(0, torch.tensor(batch_ID))
                .index_select(1, torch.tensor(fm_id))
                .index_select(2, torch.tensor(ch_IDs[0]))
            )
            f, Pxx = get_Pxx(time_w, spec_dict["srate"])
            Pxx_tensor[i, :] = Pxx
            ytick = fm
    ...
    im = ax.imshow(
        data_transform(Pxx_tensor[:, f_index[0] : f_index[1]], spec_dict["lognorm"]),
        aspect="auto",
        cmap=spec_dict["cmap"],
        vmin=extended_lims[0],
        vmax=extended_lims[1],
    )
    ...
    ax.set_xticklabels(
        [f"{value:.2f}" for value in f[f_index[0] : f_index[1] : spec_dict["groupby"][0]]],
        rotation=spec_dict["rotation"],
...
def channelsTime(input_window, Mdl, layer, spec_dict, batch_ID, FM_ID=None):
    """
    ...
    if layer is not None:
        ...
        data = torch.squeeze(
            convoluted_output.index_select(0, torch.tensor(batch_ID)).index_select(1, torch.tensor(fm_IDs))
        )
        ...
        if spec_dict["FMnames"] is None:
            ...
            if (len(spec_dict["FMnames"]) == fm) or (len(spec_dict["FMnames"]) == chan):
                ...
                raise ValueError(
                    "Length of spec_dict['FMnames'] is not equal to the FM_ID selected or to the number of channels."
                )
...
def spatialkernels_plot(ax, ax_idx1, ax_idx2, evoked_data, spec_dict, string_ID):
    ...
    evoked_data.plot_topomap(
        ...
        cbar_fmt="%.1f",
        scalings=1,
        vlim=get_extended_lims(spec_dict["vlim"]),
        contours=spec_dict["contours"],
    )
    ...
    # Set title for axes1
    title = (
        f"Filter$_{{{string_ID[0]}}}$, Input FM$_{{{string_ID[1]}}}$"
        if spec_dict["filternames"] is None
        else f"Scalp: {spec_dict['filternames'][int(ax_idx1[1] / 2)]}"
    )
    axes1.set_title(title, fontsize=spec_dict["font"])


def verticalKernel1D_scalp(
    Mdl_weights, layer, spec_dict, filter_ID=0, kernel_ID=0, kernel_width=0, MdlBase_weights=None
):
    """
    ...
    if spec_dict["norm"] == "softmax":
        ...
        if MdlBase_weights:
            ...
            spec_dict["vlim"] = (spec_dict["min_softmax"], np.max([baseweights.max().item(), weights.max().item()]))
        else:
        ...
        if MdlBase_weights:
            ...
            spec_dict["vlim"] = (
                np.min([baseweights.min().item(), weights.min().item(), spec_dict["vlim"][0]]),
                np.max([baseweights.max().item(), weights.max().item(), spec_dict["vlim"][1]]),
            )
        else:
            spec_dict["vlim"] = (
                np.min([weights.min().item(), spec_dict["vlim"][0]]),
    ...
    fig, ax = plt.subplots(
        ...
        gridspec_kw={"width_ratios": [0.9, 0.1] * filters},
        constrained_layout=True,
    )
    fig.suptitle(spec_dict["title"], fontsize=spec_dict["font"] + 2)
...
###################################
def flatten_fm(input_window, Mdl, layer, spec_dict, inout_labels, batch_ID):
    """
    ...
    fig, ax = plt.subplots(1, 1, figsize=(spec_dict["figdim"][0], spec_dict["figdim"][1]), constrained_layout=True)
    ...
    ax.set_xticklabels(
        labels=["FM-" + str(j) for j in range(0, neurons_feat, spec_dict["groupby"])],
        rotation=spec_dict["rotation"],
        fontsize=spec_dict["font"] - 4,
    )
...
def flattenCorrelationPSD(input_window, Mdl, layer, spec_dict):
    """
    ...
    for i in range(input_window.shape[0]):
        ...
        for j in range(len(band_keys)):
            ...
            bandc = data_transform(
                np.mean(np.trapz(Pxx[findex[0] : findex[1], :].T, dx=(f[1] - f[0])) / 1), spec_dict["lognorm"]
            )
    ...
    for b in range(len(band_keys)):
        ...
        axs[b].set_xlabel(
            f"Data: $\\mathcal{{F}}_X$$(${band_keys[b]}$)$" + f"$~{unit}$", fontsize=spec_dict["font"] - 2
        )
        axs[b].set_ylabel(
            f"Flatten Activation: $\\mathcal{{F}}$$(${band_keys[b]}$)$" + f"$~{unit}$", fontsize=spec_dict["font"] - 2
        )
...
####       DENSE WEIGHTS        ###
###################################
def denseweights_plot(Mdl_weights, layer, spec_dict):
    """
    ...
    if plot_type == "first":  # Only plot the weights
        ...
        fig, ax = plt.subplots(
            2, 1, figsize=(spec_dict["figdim"][0], spec_dict["figdim"][1] * 2), constrained_layout=True
        )
    ...
    if plot_type in ["first", "both"]:
        ...
        if nb_classes <= 2:
            ax[0].set_yticks([0], labels=[spec_dict["classlabels"][0]], fontsize=spec_dict["font"] - 4)
        else:
            ax[0].set_yticks(np.arange(0, nb_classes), labels=spec_dict["classlabels"], fontsize=spec_dict["font"] - 4)
...
def embedding_split2D(embedding, labels, spec_dict, ax=None):
    """
    ...
    for i in range(len(spec_dict["sets"])):
        ...
        ax.scatter(
            embedding[idxs[spec_dict["sets"][i]][0] : idxs[spec_dict["sets"][i]][1], 0],
...
def embedding_split3D(embedding, labels, spec_dict):
    ...
    for i in range(len(spec_dict["sets"])):
        ...
        scatter = ax.scatter(
            x, y, z, c=density, s=size, cmap=spec_dict["cmap"][i], alpha=alpha, label=spec_dict["sets"][i]
        )
    ...
    overlap = get_overlap3D(
        embedding,
        idxs[spec_dict["sets"][0]],
        idxs[spec_dict["sets"][1]],
        (1 - spec_dict["level"]),
        spec_dict["gridsize"],
    )
    ...
    if (
        (spec_dict["view"]["elev"] is None)
        or (spec_dict["view"]["azim"] is None)
        or (spec_dict["view"]["roll"] is None)
    ):
...
def embedding_labels2D(embedding, labels, spec_dict, ax=None):
    ...
    for i in range(len(spec_dict["classlabels"])):
        ...
        ax.scatter(
            embedding[mask, 0],
            embedding[mask, 1],
            color=spec_dict["colors"][i],
            alpha=spec_dict["alpha"],
            marker=spec_dict["marker"],
            s=spec_dict["s"],
        )
    ...
    if "dim" in spec_dict:
        ...
        kde_plot = sns.kdeplot(
            ...
            fill=False,
            levels=[spec_dict["level"]],
            palette=spec_dict["colors"],
            ax=ax,
        )
    ...
    legend_elements.append(
        Line2D([0], [0], marker="^", color="k", label=leg_label, alpha=spec_dict["alpha_triangle"], linestyle="None")
    )
...
def embedding_labels3D(embedding, labels, spec_dict):
    ...
    # Create a 3D plot
    fig, ax = plt.subplots(figsize=(spec_dict["figdim"][0], spec_dict["figdim"][1]), subplot_kw=dict(projection="3d"))
...
def embedding_class2D(embedding, inout_labels, spec_dict, subj_test=None, nw_test=None, colors_test=None, ax=None):
    """
    ...
    ax.scatter(
        embedding[test_samples:, 0],
        embedding[test_samples:, 1],
        color="k",
        alpha=spec_dict["alpha_trainval"],
        marker=spec_dict["marker"],
        s=spec_dict["s_trainval"],
    )  # VAL + TRAINING
    ...
    legend1_elements = [
        Line2D(
            ...
            alpha=spec_dict["alpha_trainval"],
            marker=spec_dict["marker"],
            markersize=spec_dict["s_trainval"],
            color="k",
            label="TRAIN+VAL",
            linestyle="None",
        )
    ...
    if spec_dict["mode"] == "subjects":
        if (nw_test is not None) and (colors_test is not None) and (subj_test is not None):
            ...
            for i, samples in enumerate(nw_test):
                ...
                ax.scatter(
                    correct[:, 0],
                    correct[:, 1],
                    color=colors_test[index],
                    alpha=spec_dict["alpha_test"],
                    marker=".",
                    s=spec_dict["s_test"],
                )
                ax.scatter(
            ...
            subject_legend = ax.legend(
                handles=legend_elements, loc=spec_dict["loc_subj"], fontsize=spec_dict["font"] - 6
            )
        ...
        legend1_elements.append(
            Line2D([0], [0], marker=".", color="k", label=f"Correct Classified: {sum(mask)}", linestyle="None")
        )
        legend1_elements.append(
            Line2D([0], [0], marker="+", color="k", label=f"Misclassified: {sum(~mask)}", linestyle="None")
        )
    ...
    # If the mode is 'classification', we plot based on classification results
        ...
        ax.scatter(
            embedding_test[mask, 0],
            embedding_test[mask, 1],
            color="g",
            alpha=spec_dict["alpha_test"],
            marker=".",
            s=spec_dict["s_test"],
        ...
        legend1_elements.append(
            Line2D([0], [0], marker=".", color="g", label=f"Correct Classified: {sum(mask)}", linestyle="None")
        )
        legend1_elements.append(
            Line2D([0], [0], marker="+", color="r", label=f"Misclassified: {sum(~mask)}", linestyle="None")
        )
...
def get_overlap3D_volume(embedding, set1_index, set2_index):
    ...
    combined_data = np.concatenate(
        [embedding[set1_index[0] : set1_index[1], :], embedding[set2_index[0] : set2_index[1], :]]
    ).T
...
def loss_correlation(ax, acc_train, acc_val, lims, spec_dict):
    ...
    ax.hist(
        res_v,
        bins=np.arange(xlim[0], xlim[1] + spec_dict["bin"], spec_dict["bin"]),
        edgecolor="black",
        color=spec_dict["color"],
        linewidth=spec_dict["linew"],
    )
...
def overfitting_inspection(models, acc_train, acc_val, spec_dict):
    ...
    for idx, (model, color) in enumerate(zip(models, spec_dict["listcolors"])):
        ...
        spec_dict.update(
            {"color": color, "xlabel": idx == len(models) - 1, "title": "" if idx != 0 else spec_dict["titlev"]}
        )
        ...
        _ = loss_correlation(
            axd[ax_pos], acc_train[model], acc_val[model], [spec_dict["xlimc"], spec_dict["ylimc"]], spec_dict
        )
