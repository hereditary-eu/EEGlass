...
function PlotScatterplot({
  ...
  function createPlot() {
    ...
    const binaryColors: Record<string, string> = {
      ...COLORS_BIN_STR,
      Unknown: "#9ca3af",
    };
    const patients_data_map =
      z_diagnosis_sel
        ? patients_data.map((patient) => ({
            ...patient,
            __diagnosis_label:
              typeof patient[categoricalFeature] === "string" && patient[categoricalFeature].trim() !== ""
                ? patient[categoricalFeature].trim()
                : "Unknown",
          }))
        : catFeatureSelected && !k_mean_cluster_sel
          ? patients_data.map((patient) => ({
              ...patient,
              __binary_label:
                patient[categoricalFeature] === 1 || patient[categoricalFeature] === "1"
                  ? "Done"
                  : patient[categoricalFeature] === 0 || patient[categoricalFeature] === "0"
                    ? "Not Done"
                    : "Unknown",
            }))
          : patients_data;
    ...
    const pd_scatterplot = Plot.plot({
      ...
      marks: [
        Plot.dot(patients_data_map, {
          ...
          ...(catFeatureSelected
            ? {
                ...(z_diagnosis_sel
                  ? {
                      stroke: "__diagnosis_label",
                    }
                  : !k_mean_cluster_sel
                    ? {
                        stroke: "__binary_label",
                      }
                  : {
        ...
        ...(showCatLinReg || showCatAvg
          ...
          : []),
        Plot.crosshair(patients_data_map, {
          x: x_feature,
          y: y_feature,
          tip: true,
        }),
      ...
      color: {
        ...(catFeatureSelected && !z_diagnosis_sel && !k_mean_cluster_sel
          ? {
              legend: true,
              domain: Object.keys(binaryColors),
              range: Object.values(binaryColors),
            }
          : {}),
        ...(z_diagnosis_sel
          ? {
  ...
  useEffect(() => {
    ...
    if (scatterplot_ref.current) {
      try {
        ...
        console.error("Scatterplot rendering failed", error);
        scatterplot_ref.current.innerHTML = "<div class=\"neuro-plot-error\">Scatterplot failed to render.</div>";
      }
    }
